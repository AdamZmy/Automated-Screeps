...
- The `screeps-world` standalone scheduled task starts a fresh conversation hourly (user updated the cadence on 2026-09-26). It reads these metrics through the local API, diagnoses persistent problems and performs authorized fixes. Quiet for unchanged or nonactionable state; notify only meaningful changes, failures, intervention results or required user action. The scheduled inspection depends on Codex execution; deployed game control does not.
