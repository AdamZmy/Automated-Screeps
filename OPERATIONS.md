...
用户2026-09-25要求：每轮发现实际异常后，必须启动一个独立的问题审视子代理；相同事件已有诊断者运行时复用，不能重复派工。审视者与修复实现者分开，给审视者充分推理时间，不要求先入为主地证明首个猜测。只传相关Issue、源码、已知故障条目与带tick的脱敏样本，记录父任务ID；root仍独占API/凭据/整合部署。

1. 按症状检索 `operations/fault-catalog.md`，优先执行对应既有检测/回归；本次命中、排除、证据不足分别记录。旧条目只是可验证假设，不能因以前发生过就断言本次同因。
2. 审视者建立假设表：机制、支持/反对证据、最小区分实验、沙盒结果、线上证据、置信状态及下一步。按采集→取货→运输→卸货→消费→续代/成本→观测口径分层，并检查多因叠加；不能把找到一个原因当作排除其他原因。
3. 若旧检查足以解释故障且修复有效，复用结论；若均排除、只解释部分症状、有矛盾或修复后异常持续，则投入深入推理扩展新假设。能源积压至少考虑消费WORK/预算/补给、合法多卸货位与占位、预留/批量/目标切换、逐矿运力与全局池、身体/路线/疲劳/寿命、源再生/衰减/账本残差/CPU。按矿固定与更大体型都是待验证方案，不默认优于动态策略。
4. 在隔离沙盒复现，以现有引擎常量、真实地形、可达性和多tick状态验证；成功返回码不是实际行为证据。root补采线上证据并做受控修复，比较真实转入/消费、完整往返周期、矿区库存与CPU。策略体型同时看孵化容量、当前能量、可持续收入、角色需求和生命周期成本，不一律花满容量。
5. 只有代码/沙盒可复现或新鲜运行证据确认的机制，才新增/更新已知故障条目；标明“机制已确认、是否解释本次/是否修复仍待验收”。条目至少含ID、触发条件、最小观测、判断/排除条件、复现命令、安全处置、回归与版本/Issue证据。猜测和当轮排除项留诊断报告，不混入已确认目录；排除结论只对记录条件和tick有效。
6. 修复后由审视者独立复核因果与残留异常；固定回归必须实际运行，不能只增加文档。新知识写目录、回归测试、原Issue及本轮日志，报告按症状保存在 `operations/diagnostics/`。下轮先读目录相关项和Issue，不重读全部报告或从零推理。确有新证据可修订/废弃旧条目。

有限代理须交付并释放文件；没有异常时运行轻量检查，不凭空制造研究任务。诊断不阻止root处理已经明确的紧急故障，但紧急处理后仍要做独立审视与残留验证。
